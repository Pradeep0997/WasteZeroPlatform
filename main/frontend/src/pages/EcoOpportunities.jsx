...
const EcoOpportunities = () => {
  ...
  // Determine role — admins can view but not apply
  ...
  const loadOpportunities = async () => {
    try {
      ...
      const promises = [volunteerAPI.getAllOpportunities(params)];

      if (currentUserRole === 'volunteer') {
        promises.push(volunteerAPI.getMyApplications().catch(() => ({ data: [] })));
      }

      const results = await Promise.all(promises);
      const oppResponse = results[0];
      const appResponse = results.length > 1 ? results[1] : null;
      ...
      if (appResponse) {
        setApplications(appResponse.data || appResponse || []);
      }
    } catch (error) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredOpportunities.map((opportunity) => {
              const isApplied = applications.some((app) =>
                (app.opportunityId?._id || app.opportunityId || app.opportunity?._id || app.opportunity) === opportunity._id
              );
              return (
                <div
                  key={opportunity._id}
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${opportunity.isMatched
                    ...
                    {currentUserRole !== 'volunteer' ? (
                      <div className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded-lg text-center text-sm font-medium border border-gray-200 dark:border-gray-600">
                        👁 View Only ({currentUserRole === 'ngo' ? 'NGO' : 'Admin'})
                      </div>
                    ) : (
                      <button
                        onClick={() => handleApply(opportunity._id)}
                        disabled={isApplied || applyingTo === opportunity._id}
                        className={`w-full px-4 py-2 font-medium rounded-lg transition-colors ${isApplied
                            ? 'bg-gray-100 text-gray-500 cursor-not-allowed dark:bg-gray-700 dark:text-gray-400'
                            : 'bg-green-600 text-white hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed'
                          }`}
                      >
                        {applyingTo === opportunity._id ? 'Applying...' : (isApplied ? 'Already Applied' : 'Apply Now')}
                      </button>
                    )}
                  </div>
                </div>
