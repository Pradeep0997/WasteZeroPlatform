...
const WasteZeroAnalytics = ({ userRole = 'volunteer', userId = null }) => {
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      // Using hardcoded data to ensure dashboard is fully populated
      setTimeout(() => {
  ...
  const generateFallbackData = () => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      monthlyData.push({
        ...
        volunteers: Math.floor(Math.random() * 4) + 1,
    ...
    const activityTypeData = [
      { name: 'Environmental', value: 45, count: 8 },
      { name: 'Community', value: 25, count: 5 },
      { name: 'Education', value: 20, count: 4 },
      { name: 'Health', value: 10, count: 2 }
    ];
    ...
    const topVolunteers = [
      { id: 1, name: 'Alice Johnson', eventsParticipated: 4, totalHours: 18 },
      { id: 2, name: 'Bob Smith', eventsParticipated: 3, totalHours: 14 },
      { id: 3, name: 'Carol Davis', eventsParticipated: 2, totalHours: 11 },
      { id: 4, name: 'David Wilson', eventsParticipated: 2, totalHours: 9 },
      { id: 5, name: 'Emma Brown', eventsParticipated: 1, totalHours: 4 }
    ];

    const topNGOs = [
      { id: 1, name: 'Green Earth Initiative', totalEvents: 4 },
      { id: 2, name: 'Ocean Cleaners', totalEvents: 3 },
      { id: 3, name: 'City Planters', totalEvents: 2 },
      { id: 4, name: 'Eco Warriors', totalEvents: 2 },
      { id: 5, name: 'Wildlife Rescuers', totalEvents: 1 }
    ];

    const recentActivities = [
      { id: 1, event: 'Community Garden Project', type: 'Event Created', status: 'active', date: new Date(), location: 'Green Valley' },
      { id: 2, event: 'River Cleanup Drive', type: 'Application', status: 'pending', date: new Date(), location: 'Riverside Park' },
      { id: 3, event: 'Recycling Workshop', type: 'Event Created', status: 'active', date: new Date(), location: 'Community Center' },
      { id: 4, event: 'Tree Plantation', type: 'Application', status: 'accepted', date: new Date(), location: 'City Outskirts' },
      { id: 5, event: 'E-Waste Collection', type: 'User Registration', status: 'accepted', date: new Date(), location: 'Tech Park' },
      { id: 6, event: 'Beach Cleanup', type: 'Event Created', status: 'active', date: new Date(), location: 'Sunny Beach' }
    ];
                ...
                selectedUser.recentActivities.map((activity, index) => (
                  <div key={index} className="bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-700 dark:to-blue-900/20 rounded-xl p-4 border-l-4 border-blue-500 hover:shadow-md transition-all">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`text-xs px-3 py-1 rounded-full font-bold ml-3 ${activity.status === 'accepted' || activity.status === 'active' ? 'bg-emerald-500 text-white' :
              ...
              (analyticsData?.recentActivities || []).map((activity, index) => (
                <div key={index} className="bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-900/20 dark:to-cyan-900/20 rounded-xl p-4 hover:shadow-lg transition-all border-l-4 border-teal-500">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className={`text-xs px-3 py-1 rounded-full font-bold ml-3 ${activity.status === 'accepted' ? 'bg-emerald-500 text-white' :
                      activity.status === 'pending' ? 'bg-amber-500 text-white' :
                        'bg-rose-500 text-white'
                      }`}>
                      {activity.status}
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-2 flex-wrap">
                        {activity.type && (
                          <span className={`text-xs px-2 py-1 rounded-full font-medium ${activity.type === 'Event Created' ? 'bg-emerald-500 text-white' :
                    ...
                    <div className="text-right ml-3">
                      {activity.status && (
                        <span className={`text-xs px-3 py-1 rounded-full font-bold ${activity.status === 'accepted' || activity.status === 'completed' || activity.status === 'active' ? 'bg-emerald-500 text-white' :
