      ...
      try {
        ...
        if (response.success && response.data?.users) {
          const mappedUsers = response.data.users.map(u => ({
            ...
            name: u.name || "Unknown",
            email: u.email || "",
            username: u.name ? u.name.split(" ")[0].toLowerCase() + Math.floor(Math.random() * 100) : "user123",
            status: u.isBlocked ? "Banned" : (u.status || "Active"),
            role: u.role ? u.role.charAt(0).toUpperCase() + u.role.slice(1) : "User",
            joined: u.createdAt ? new Date(u.createdAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) : "October 1, 2025",
            lastActive: "Just now",
            originalUser: u
          }));
