...
export default function VolunteerDashboard() {
    ...
    const getAppliedIds = () => {
        return applications.map(app => app.opportunityId?._id || app.opportunityId || app.opportunity?._id || app.opportunity);
    };

    const appliedIds = getAppliedIds();

    const hasApplied = (opportunityId) => appliedIds.includes(opportunityId);
    ...
    const renderDashboardTab = () => (
        <div className="space-y-8 p-6 lg:p-8 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border">
                ...
                <MatchedOpportunities onApply={handleApply} loading={loading} appliedIds={appliedIds} />
            </div>
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredOpportunities.map(op => {
                    const isApplied = hasApplied(op._id);
                    return (
                        <div key={op._id} className="bg-white rounded-xl shadow p-4 flex flex-col">
                            <div className="flex-grow">
                                <h4 className="font-semibold">{op.title}</h4>
                                <p className="text-sm text-gray-600 line-clamp-2 mt-1">{op.description}</p>
                            </div>
                            <div className="mt-4 flex items-center justify-between">
                                <button
                                    onClick={() => handleApply(op._id)}
                                    disabled={isApplied || applyingTo === op._id}
                                    className={`px-4 py-2 font-medium rounded-lg transition-colors ${isApplied
                                            ? 'bg-gray-100 text-gray-500 cursor-not-allowed'
                                            : 'bg-green-600 text-white hover:bg-green-700'
                                        }`}
                                >
                                    {applyingTo === op._id ? 'Applying...' : (isApplied ? 'Already Applied' : 'Apply')}
                                </button>
                            </div>
                        </div>
                    )
