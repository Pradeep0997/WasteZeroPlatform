...
const AttendanceManager = ({ eventId, onClose }) => {
  ...
  const markAttendance = useCallback(async (volunteerId, status, notes = '') => {
    if (!attendanceData?.event?.date) return;

    // Check if event date is today or strictly past
    const today = new Date();
    const eventDate = new Date(attendanceData.event.date);

    const todayOnly = new Date(today.getFullYear(), today.getMonth(), today.getDate());
    const eventDateOnly = new Date(eventDate.getFullYear(), eventDate.getMonth(), eventDate.getDate());

    if (todayOnly < eventDateOnly) {
      showNotification('Event date is not today', 'error');
      return;
    }
  ...
  }, [eventId, loadAttendanceData, showNotification, attendanceData]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4 md:p-8">
                ...
                <tbody>
                  {volunteers.map((volunteer, index) => (
                    <tr key={volunteer.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide ${volunteer.attendanceStatus === 'present' ? 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300' :
      ...
      {notifications.map(n => (
        <div
          key={n.id}
          className={`fixed bottom-6 right-6 p-4 rounded-xl shadow-2xl z-50 backdrop-blur-sm animate-slide-up ${n.type === 'success' ? 'bg-green-500/90 text-white' :
