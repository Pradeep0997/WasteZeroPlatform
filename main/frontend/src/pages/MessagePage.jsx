...
export default function MessagePage() {
    ...
    useEffect(() => {
        if (currentUser) {
            ...
            newSocket.on("newMessage", (newMessage) => {
                ...
                if (selectedId && selectedId === senderId) {
        ...
        const getUsers = async () => {
            try {
                ...
                const filtered = users.filter(u => {
                    const isCurrentUser = (u._id || u.id) === (currentUser._id || currentUser.id);
                    if (isCurrentUser) return false;

                    if (u.role === 'admin' && u.name.toLowerCase() !== 'super admin' && u.name.toLowerCase() !== 'admin2') {
                        return false;
                    }
                    return true;
                });
                ...
                filtered.forEach(u => {
                    if (u.unreadCount > 0) {
                        initialUnread[u._id || u.id] = u.unreadCount;
                    }
                });
    ...
    useEffect(() => {
        ...
        const getMessages = async () => {
            try {
                // 👇 Use the imported API service
                const data = await messageAPI.fetchMessages(selectedUser._id || selectedUser.id);
                setMessages(data);
            } catch (err) {
    ...
    const sendMessage = async (e) => {
        ...
        try {
            const selectedId = selectedUser._id || selectedUser.id;
            const sentMessage = await messageAPI.postMessage(selectedId, newMsg);
