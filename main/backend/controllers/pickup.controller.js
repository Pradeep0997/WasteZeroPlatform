...
const getMyPickups = async (req, res) => {
  try {
    ...
    if (req.user.role === 'volunteer') {
      ...
      query.$or = [
        { requestedBy: userId },           // pickups this NGO created
        { assignedTo: userId },            // pickups assigned to this NGO
        { assignedTo: null, status: 'pending' } // all pending requests for NGO to accept
      ];
...
const getPickupById = async (req, res) => {
  try {
    ...
    const hasPermission =
      pickup.requestedBy._id.toString() === (req.user._id || req.user.id).toString() ||
      pickup.assignedTo?._id?.toString() === (req.user._id || req.user.id).toString() ||
      pickup.agent?._id?.toString() === (req.user._id || req.user.id).toString() ||
      req.user.role === 'admin';
...
const updatePickup = async (req, res) => {
  try {
    ...
    // Check permissions
    const canEdit =
      pickup.requestedBy.toString() === (req.user._id || req.user.id).toString() ||
      pickup.assignedTo?.toString() === (req.user._id || req.user.id).toString() ||
      req.user.role === 'admin';
...
const acceptPickup = async (req, res) => {
  try {
    ...
    pickup.assignedTo = req.user._id || req.user.id;
...
const assignAgent = async (req, res) => {
  try {
    ...
    if (pickup.assignedTo.toString() !== (req.user._id || req.user.id).toString()) {
      return res.status(403).json({
        success: false,
...
const cancelPickup = async (req, res) => {
  try {
    ...
    [pickup.requestedBy, pickup.assignedTo, pickup.agent].forEach(user => {
      if (user && user._id.toString() !== (req.user._id || req.user.id).toString()) {
...
const updatePickupStatus = async (req, res) => {
  try {
    ...
    if (pickup.agent?._id?.toString() !== (req.user._id || req.user.id).toString() && req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
...
const getPickupStats = async (req, res) => {
  try {
    ...
    // Filter based on user role
    if (req.user.role === 'volunteer') {
      matchQuery.requestedBy = req.user._id || req.user.id;
    } else if (req.user.role === 'ngo') {
      matchQuery.assignedTo = req.user._id || req.user.id;
    }
