...
const getAllOpportunities = async (req, res) => {
  try {
    ...
    if (includeMatched === 'true' && req.user && (req.user._id || req.user.id) && req.user.role === 'volunteer') {
      try {
        const matches = await matchingService.findMatchingOpportunities(req.user._id || req.user.id, 50);
  ...
  try {
    ...
    if (req.user.role !== 'volunteer') {
      return res.status(403).json({
        success: false,
        message: 'Only registered volunteers can apply for opportunities.'
      });
    }
...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Get upcoming events (accepted applications for future events) — correct approach
    const acceptedApps = await Application.find({ volunteerId, status: 'accepted' })
      .populate({ path: 'opportunityId', select: 'date' });
    const upcomingEvents = acceptedApps.filter(
      a => a.opportunityId?.date && new Date(a.opportunityId.date) >= new Date()
    ).length;
