...
const sendMessage = async (req, res) => {
    try {
        ...
        const newMessage = new Message({ senderId: senderIdStr, receiverId: receiverIdStr, message });
...
const getMessages = async (req, res) => {
    try {
        ...
        const senderIdStr = senderId.toString();
        const userToChatIdStr = userToChatId.toString();

        // Find conversation containing both users
        const conversation = await Conversation.findOne({
            participants: { $all: [senderIdStr, userToChatIdStr] },
