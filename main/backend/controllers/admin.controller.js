...
const getUserDetails = async (req, res) => {
  try {
    ...
    if (user.role === 'ngo') {
      ...
      const totalApplications = await Application.countDocuments({
        opportunityId: { $in: opportunities.map(opp => opp._id) } // FIX: was 'opportunity'
      });
      ...
      const applications = await Application.find({ volunteerId: userId }); // FIX: was 'volunteer'
      const acceptedApplications = applications.filter(app => app.status === 'accepted');

      additionalStats = {
        totalApplications: applications.length,
        acceptedApplications: acceptedApplications.length,
        completedEvents: acceptedApplications.length
      };
    }
...
const toggleUserBlock = async (req, res) => {
  try {
    ...
    user.blockedBy = user.isBlocked ? (req.user._id || req.user.id) : null;
