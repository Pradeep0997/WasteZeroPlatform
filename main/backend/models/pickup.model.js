...
// Note: geospatial ($near) queries require GeoJSON Point format.
// Current schema stores coordinates as {latitude, longitude} (plain numbers),
// so we use a standard compound index instead of 2dsphere.
pickupSchema.index({ 'location.coordinates.latitude': 1, 'location.coordinates.longitude': 1 });
...
// Static method to find pickups within a rough bounding box by lat/lng
// (Uses plain {latitude, longitude} schema — not GeoJSON, so $near is not available)
pickupSchema.statics.findByLocation = function (lat, lng, radiusInKm = 10) {
  const degreeOffset = radiusInKm / 111; // ~111 km per degree
  return this.find({
    'location.coordinates.latitude': { $gte: lat - degreeOffset, $lte: lat + degreeOffset },
    'location.coordinates.longitude': { $gte: lng - degreeOffset, $lte: lng + degreeOffset },
  });
};

// Pre-save middleware to update timestamps
pickupSchema.pre('save', function (next) {
  if (this.status === 'completed' && this.completionDetails && !this.completionDetails.completedAt) {
